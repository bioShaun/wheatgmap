...
function createTable(headData, bodyData, href) {
  var htmlBuffer = [];
  htmlBuffer.push("<table class='table table-strip table-bordered enrich-table'>");
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (data.msg == "ok") {
        ...
        $('#result-table').empty();
        var tableStr = createTable(data.result.header, data.result.body, data.result.href);
        $('#result-table').html(tableStr);
        $('.enrich-table').DataTable({
            dom: 'Bfrtip',
            buttons: [
