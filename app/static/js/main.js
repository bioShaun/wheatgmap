...
function createHref(gene) {
  if (gene.split("-").length == 2) {
    ...
    href_str1 =
      "<a href='/tools/gene/information/?gene=" +
      gene_array[0] +
      "'>" +
      gene_array[0] +
      "</a>";
    ...
    href_str =
      "<a href='/tools/gene/information/?gene=" + gene + "'>" + gene + "</a>";
  }
  return href_str;
}
...
function select_plugin() {
  ...
  $(".multi_d").multiselect({
    ...
    leftSelected:
      "#multi_d_leftSelected, #multi_d_leftSelected_2, #multi_d_leftSelected_3, #multi_d_leftSelected_4, #multi_d_leftSelected_5",
    rightAll:
      "#multi_d_rightAll, #multi_d_rightAll_2, #multi_d_rightAll_3, #multi_d_rightAll_4, #multi_d_rightAll_5",
    leftAll:
      ...
      if (button == "multi_d_rightSelected") {
        ...
        if (typeof Multiselect.callbacks.sort == "function" && !silent) {
          Multiselect.$right
            ...
            .find("> option")
            .sort(Multiselect.callbacks.sort)
            .appendTo(Multiselect.$right.eq(2));
        }
        ...
        if (typeof Multiselect.callbacks.sort == "function" && !silent) {
          Multiselect.$right
            ...
            .find("> option")
            .sort(Multiselect.callbacks.sort)
            .appendTo(Multiselect.$right.eq(3));
        }
        ...
        if (typeof Multiselect.callbacks.sort == "function" && !silent) {
          Multiselect.$right
            ...
            .find("> option")
            .sort(Multiselect.callbacks.sort)
            .appendTo(Multiselect.$right.eq(4));
        }
...
function createPlotData(groupA, groupB, data) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    (tmpObj["type"] = "line"), (tmpObj["data"] = line);
    series.push(tmpObj);
  }
  var plotInfo = {
    gene_name: gene_ids,
...
function createPlot(files, name, params) {
  ...
  for (var i = 0; i < files.length; i++) {
    liBuffer.push(
      "<li><a href='" +
        files[i] +
        "' class='snp-index' title='' rel='example_group3'><img src='" +
        files[i] +
        "'></a></li>"
    );
  }
...
function check_location(info, maxRange = 1000000) {
  keys = Object.keys(info);
  for (var i = 0; i < keys.length; i++) {
    if (info[keys[i]].length == 0) {
      return keys[i] + " is empty!";
    }
  }
  if (info["end-position"] <= info["start-position"]) {
    return "Start position >= End position";
  }
  if (maxRange) {
    if (info["end-position"] - info["start-position"] > maxRange) {
      return "Range length > 1Mb!";
    }
  }
  return "";
}
