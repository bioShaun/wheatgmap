...
def fetch_blast_table():
    if request.method == "POST":
        ...
        if "," in gene_str:
            gene_list1 = list(
                itertools.chain(*[each.split(",") for each in gene_str.split()])
            )
        else:
...
def fetch_exp():
    if request.method == "POST":
        ...
        if len(tissue) >= 5:
            pearson_res = pearson_cor(
                mrna, lncrna, mrna_exp_arr, lnc_exp_arr, mrna_lnc_pair
            )
        else:
            pearson_res = []
