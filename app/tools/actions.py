...
def get_locus_result(genename, blast_results):
    ...
    if result:
        ...
        if ortho_result:
            ...
            ortho_result_list = [
                addOuterLink(each) for each in ortho_result_list if each is not None
            ]
...
def fetch_sequence(table, chr, start_pos, end_pos):
    ...
    --chrom {chr} \
    --start_pos {start_pos} \
    --end_pos {end_pos}".format(
        script=vcf_seq_script,
...
def run_pca(filename, sample_group):
    ...
    if sample_group:
        sample_df = _pandas_read(
            os.path.join(UPLOAD_FOLDER, "group", sample_group), header=None
        )
...
def fetch_lnc_pcg_pairs(options):
    ...
    if genes:
        mrna_res = RnaNeighbor.query.filter(RnaNeighbor.mRNA_gene.in_(genes)).all()
        lnc_res = RnaNeighbor.query.filter(RnaNeighbor.lncRNA_gene.in_(genes)).all()
    else:
        chrom = options["chrom"]
        start = options["start"]
        end = options["end"]
        mrna_res = RnaNeighbor.query.filter(
            RnaNeighbor.chrom == chrom,
            RnaNeighbor.mRNA_start >= start,
            RnaNeighbor.mRNA_end <= end,
        ).all()
        lnc_res = RnaNeighbor.query.filter(
            RnaNeighbor.chrom == chrom,
            RnaNeighbor.lncRNA_start >= start,
            RnaNeighbor.lncRNA_end <= end,
        ).all()
...
def fetch_pcg_exp(geneList: List[str], sampleList: List[str]):
    query_out = GeneExpression.query.filter(
        GeneExpression.gene_id.in_(geneList), GeneExpression.tissue.in_(sampleList)
    ).all()
...
def fetch_lnc_exp(geneList: List[str], sampleList: List[str]):
    query_out = LncExpression.query.filter(
        LncExpression.gene_id.in_(geneList), LncExpression.tissue.in_(sampleList)
    ).all()
    return [item.as_dict() for item in query_out]
