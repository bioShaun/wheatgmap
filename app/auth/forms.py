...
def legal_username_check(form, field):
    if not re.match("^[0-9a-zA-Z_]+$", field.data):
        raise ValidationError("Only Alphabets, Numbers and Underscore are allowed.")


class RegisterForm(FlaskForm):

    username = StringField(
        "Username", validators=[DataRequired(), Length(3, 80), legal_username_check]
    )
    ...
    institute = StringField("Institute", validators=[DataRequired(), Length(1, 80)])
    telephone = StringField("Telephone", validators=[DataRequired(), Length(1, 20)])
    pub_phone = SelectField(
        "Public Telephone", choices=[(0, "No"), (1, "Yes")], coerce=int
    )
