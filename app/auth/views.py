...
def register():
    ...
    if form.validate_on_submit():
        ...
        send_mail(
            user.email, "Confirm Your Account", "mail/confirm", user=user, token=token
        )
        flash(
    ...
    return render_template(
        "auth/edit.html", form=form, defaultPhoto=Config.DEFAULT_USER_PHOTO
    )
...
def tasks():
    ...
    my_tasks = (
        TaskInfo.query.filter_by(username=current_user.username)
        .order_by(TaskInfo.create_time.desc())
        .all()
    )
...
def upload():
    if request.method == "POST":
        ...
        if vcf_file and re.search(".vcf.gz$", vcf_file.filename):
            # add timestamp
            filename = "_".join(
                [str(int(time.time())), secure_filename(vcf_file.filename)]
            )
            ...
            if current_user.is_authenticated:
                ...
                username = "anonymouse"
            task = async_fetch_vcf_samples.delay(
                filename, username, upload_id, vcf_type
            )
            appUitls.redis_task.push_task(username, task.id)
            upload_task = TaskInfo(
                task_name=vcf_file.filename,
...
def userData():
    ...
    pu_vcf = 0
    pr_sample = len(
        Data.query.filter_by(opened=0, sign=0, provider=current_user.username).all()
    )
    pu_sample = len(
        Data.query.filter_by(opened=1, sign=0, provider=current_user.username).all()
    )
    stat = {
        "pr_vcf": pr_vcf,
        "pu_vcf": pu_vcf,
...
def fetch_samples():
    if request.method != "GET":
        ...
        appUitls.logger().info(
            "cached delete: wheatgmap.{0}.data".format(current_user.username)
        )
    if request.method == "GET":
        result = []
