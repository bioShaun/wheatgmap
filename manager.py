...
from app.auth.models import User, GeneExpression, RnaNeighbor
...
def add_exp():
    print('read data')
    df = pd.read_csv(
        '/data/wheatdb/data/expression/iwgsc.gene.tpm.grp.long_table.csv.gz')
    print('load data')
    geneNum = len(df.index)
    unitNum = 10000
    intervals = zip(range(0, geneNum, unitNum),
